...
(function() {
  var LocalStrategy, MailService, Tokens, TrelloApi, TrelloOAuth, TrelloView, Trellos, Users, app, authRequired, cluster, config, express, http, i, notificationLoop, passport, server, _, _i;
  ...
  app.get('/password/reset', function(req, res) {
    return Users.findByResetPasswordToken(req.query.token, function(err, user) {
      if (err) {
  ...
  app.post('/password/reset', function(req, res) {
    return Users.findByEmail(req.body.email, function(err, user) {
      if (err) {
        return app.handleError(err, res);
      }
      return user.requestPasswordReset(function(err, result) {
        if (err) {
          return app.handleError(err, res);
        }
  ...
  app.put('/password', function(req, res) {
    return Users.findByResetPasswordToken(req.body.token, function(err, user) {
      ...
      return user.resetPassword(req.body.password, req.body.password_confirmation, function(err, user) {
        if (err) {
          return app.handleError(err, res);
        }
        return res.json({
