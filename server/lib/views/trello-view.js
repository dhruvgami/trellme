...
(function() {
  var Cards, ObjectID, TrelloView, Trellos, fs, handlebars, should, sugar, _;
  ...
  module.exports = TrelloView = (function() {
    ...
    TrelloView.prototype.collectCardsPerDue = function(cards) {
      var due, now,
        _this = this;
      now = new Date();
      due = {
      ...
      _.each(cards, function(card) {
        return _.each(card.cards, function(kard) {
          var diff, dueDate;
          if (kard.due) {
            dueDate = new Date(kard.due);
            diff = (dueDate - now) / 86400000;
            if (dueDate.sameDateAs(now)) {
              return due.today.push(card.id);
            } else {
              if (diff < 0) {
                return due.past.push(kard.id);
              } else if (diff < 7) {
                return due.soon.push(kard.id);
              }
            }
          }
        });
    ...
    TrelloView.prototype.getReports = function(userId, cb) {
      ...
      return Trellos.getAllData(userId, function(err, data) {
        ...
        dueCards = _this.collectCardsPerDue(data.cards);
